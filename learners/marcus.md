...
Is there a correlation between the average temperature and average rainfall over the measured period. 

## Hypothesis

A higher temperature could effect water evaporation, leading to more rainfall. 
...
This is a simple test where we do not take larger wetherevents in to account. Nor can we way anything about cusal effects. 
...
--------------|--------------------------------------------------------------------|----------------------|-------------------------------------------------------------------------------------------------------------------------------|-------------------------------------|------------------------|-----------|-------------|-------|
R1            |Reading the data file follows best practices                        |Passes the test case  |The `.csv` file is parsed correctly by a function called `read_data` | XX | 2 | 3 | Validation function for input 
R1.1          |The data file can be read                                           |Passes the test case  |The content of the file can be read without errors | Wrong input | XX | 2 | 3 | Validation function for input 
R1.2          |Reading the data file produces a table                              |Passes the test case  |Reading the existing data file (in `/data`) produces a table | Module comptatibility issues, imporperly loaded packages | 3 | 3 | Use the approproate library for this data. | 
R1.2          |The table produced from reading the data has correct column names   |Passes the test case  |The column names must match between file and the table that is created by reading the file | XX | 1 | 4 | Add a function whihc doubble checks that the names are the same.  | 
R1.3          |The table produced from reading the data has the correct content    |Passes the test case  |The column names must match between file and the table that is created by reading the file | XX | 1 | 4 | Add a function that makes sure that hte table has the right number of rows and columns.  | 
.             |.                                                                   |Passes the test case  |The content of the first row of the table created by loading the file, matches exactly the content of the first row in the file | xx | 1 | 4 | Compare teh first row of the table to haed coded values. | 
R1.4          |Read data file is read in a short time                              |Passes the test case  |The file is read within 1 second  | xx | 1 | 1 | Implement resonable function to import the data |
R1.5          |Reading an absent file gives an error                               |Passes the test case  |If the file cannot be found, create a helpful error message, that includes the path of the absent file |  | 1 | 2 | Add some helpful error message |
R1.6          |Table column names are documented                                   |Passes the test case  |The documentation contains the names of the columns | xx | 3 | 3 | Ensure that we have proper documentation. Add a funcion that doubble checks the content of our metadata file.  |
