Hypothesis:
---
Does the average temperature increase over the year?
What kind of function can be fit with the temperature increase?
...
R1.1          |The data file can be read                                           |Passes the test case  |The content of the file can be read without errors |File will not be read in cases where - other delimeters than comma is used in the input file; The file size (i.e. number of characters to be read) is out of range of the read_data (nxp) which reads the data file; P5S2 - Issue
R1.2          |Reading the data file produces a table                              |Passes the test case  |Reading the existing data file (in `/data`) produces a table
R1.2          |The table produced from reading the data has correct column names   |Passes the test case  |The column names must match between file and the table that is created by reading the file | Given the data and metadata file, P1S1
R1.3          |The table produced from reading the data has the correct content    |Passes the test case  |The column names must match between file and the table that is created by reading the file | Given the data and metadata file, P1S1
...
R1.4          |Read data file is read in a short time                              |Passes the test case  |The file is read within 1 second | P1S1
...
R1.6          |Table column names are documented                                   |Passes the test case  |The documentation contains the names of the columns | P1S2 given the metadata file here
...
R100          |Best practices are followed                                         |All team members agree|We think we follow the best practices as found in the academic literature | Business risk
