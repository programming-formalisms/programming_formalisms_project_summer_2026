...
Requirement ID|Requirement description                                             |Acceptance criteria   |Test cases | Risks |                                           
--------------|--------------------------------------------------------------------|----------------------|------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------|
R1            |Reading the data file follows best practices                        |Passes the test case  |The `.csv` file is parsed correctly by a function called `read_data` | P5S2: relatively rare event. |
R1.1          |The data file can be read                                           |Passes the test case  |The content of the file can be read without errors. | P5S2: relatively rare event. OBS :`read_data` has nxp matrix load limitations. In case user has a larger data frame that what is acceptable by `read_data` the loading will fail. |
R1.2          |Reading the data file produces a table                              |Passes the test case  |Reading the existing data file (in `/data`) produces a table | P1S1: Since the data and the metadata file is already provided, all discrepancies is already solved |
R1.2          |The table produced from reading the data has correct column names   |Passes the test case  |The column names must match between file and the table that is created by reading the file | P1S1: Since the data and the metadata file is already provided, all discrepancies is already solved |
R1.3          |The table produced from reading the data has the correct content    |Passes the test case  |The column names must match between file and the table that is created by reading the file | P1S1: Since the data and the metadata file is already provided, all discrepancies is already solved |
.             |.                                                                   |Passes the test case  |The content of the first row of the table created by loading the file, matches exactly the content of the first row in the file | P1S1: Since the data and the metadata file is already provided, all discrepancies is already solved |
R1.4          |Read data file is read in a short time                              |Passes the test case  |The file is read within 1 second | P1S1: Since the data and the metadata file is already provided, all discrepancies is already solved |
R1.5          |Reading an absent file gives an error                               |Passes the test case  |If the file cannot be found, create a helpful error message, that includes the path of the absent file | P1S2 |
R1.6          |Table column names are documented                                   |Passes the test case  |The documentation contains the names of the columns | P1S2: metadata and col values in data should be aggreeable for cross-validation |
.             |.                                                                   |Two humans agree      |The documentation describes the content of the columns | P1S1 |
